...
def index(member_id):
    ...
    if request.method == "POST":
        ...
        if not password and not member_id:
            errors['password']= 'You must hava a password!'
        if not location:
        ...
        if not interest_in_topics:
            errors['interest_in_topics'] = 'Yopu must select a topic'
        ...
        if not errors:
            if member:
                ...
                member.about=about
                member.earn_new_interest=(True if learn_new_interest == "yes" else False)
